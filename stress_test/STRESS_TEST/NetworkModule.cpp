...
void Adjust_Number_Of_Client()
{
	...
	if (ACCEPT_DELY * delay_multiplier > duration_cast<milliseconds>(duration).count())
	{
		return;
	}
	...
	if (active_clients > max_limit - max_limit / 20)
	{
		return;
	}
	...
	g_clients[idx].client_socket = sock;
	...
	if (WSAConnect(sock, (sockaddr*)&ServerAddr, sizeof(ServerAddr), NULL, NULL, NULL, NULL) != 0) 
	{
	...
	// IOCP 등록
	CreateIoCompletionPort((HANDLE)sock, g_hiocp, idx, 0);
	...
	int r = WSARecv(
		sock,
		&g_clients[idx].recv_over.wsabuf,
		1,
		NULL,
		&recv_flag,
		&g_clients[idx].recv_over.over,
		NULL
	);

	if (r == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) 
	{
		error_display("WSARecv failed: ", WSAGetLastError());
		closesocket(sock);
		return;
	}
	...
	ZeroMemory(&l_packet, sizeof(l_packet));
	l_packet.size = sizeof(l_packet);
	l_packet.type = CS_LOGIN;
	sprintf_s(l_packet.name, "%d", idx);
	SendPacket(idx, &l_packet);
